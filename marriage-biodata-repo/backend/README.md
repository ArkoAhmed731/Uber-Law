...
## Prerequisites
- **Node.js 18+ (includes npm)**: Install from https://nodejs.org. On Windows you can run `winget install OpenJS.NodeJS.LTS` or `choco install nodejs-lts`, then restart your terminal/VS Code. Confirm installation with `node -v` and `npm -v`; if you see `'npm' is not recognized`, ensure `C:\\Program Files\\nodejs\\` is on your PATH or reinstall Node.
- **MySQL**: Set your connection string in `.env` (copy from `.env.example`).

## Setup

```bash
cd marriage-biodata-repo/backend
npm install

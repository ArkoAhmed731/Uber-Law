...
## Prerequisites
- **Node.js 18+ (includes npm)**: Install from https://nodejs.org or on Windows run `winget install OpenJS.NodeJS.LTS` (or `choco install nodejs-lts` if you use Chocolatey). After installing, restart VS Code/Command Prompt so the updated `PATH` is picked up. Verify with `node -v` and `npm -v`—if you see "'npm' is not recognized", add `C:\Program Files\nodejs\` to your PATH or reinstall Node.
- **MySQL**: Provide a connection string in `backend/.env`.
...
1. Copy or move this folder out of the old project (e.g., `cp -R marriage-biodata-repo ~/marriage-biodata-repo`).
...
If you cannot move it yet, still run `git init` inside `marriage-biodata-repo` to keep its history separate from the parent repo.

## Quick start (after creating the new repo)
- Backend: `cd backend && npm install && npm run prisma:migrate && npm run db:seed && npm run dev`
